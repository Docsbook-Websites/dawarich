...
There is a known issue when data migrations are not being run automatically on some systems. If you're experiencing issues when opening map page, trips page or when trying to see visits, try executing the following command in the [Console](../faq.md#how-to-enter-dawarich-console):
...
This means that the data migration job that supposed to move coordinates from `longitude` and `latitude` columns to `lonlat` column on `points` table was not run. We can fix this by running the following command in the [Console](../faq.md#how-to-enter-dawarich-console):
...
This means, you have some points with the same coordinates, timestamp and user_id. This usually should not happen, but we can remove the duplicates. Run the following command in the [Console](../faq.md#how-to-enter-dawarich-console):
...
In the [Console](../faq.md#how-to-enter-dawarich-console), run the following command:
...
Also, have a look at the [moving to postgis](./maintenance/moving-to-postgis.md) guide.
